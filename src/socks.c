...
void log_message(int level, const char *message, ...) {
  ...
  if (errno != 0) {
    fprintf(log_file, "%s[%s][%lu] Critical: %s - %s%s\n", color_critical, date,
            self, vbuffer, strerror(errno), color_reset);
    errno = 0;
  } else if (!level) {
    fprintf(log_file, "%s[%s][%lu] Error: %s%s\n", color_error, date, self,
            vbuffer, color_reset);
  } else if (level) {
    fprintf(log_file, "%s[%s][%lu] Info: %s%s\n", color_info, date, self,
            vbuffer, color_reset);
  } else {
...
void socks5_auth(int fd, int methods) {
  ...
  case USERPASS: {
    ...
    if (!strcmp(socks_username, username) &&
        !strcmp(socks_password, password)) {
...
void socks_send_replies(int fd, int rep, address_t *addr) {
  ...
  log_info("Replies %hhX %hhX %hhX %hhX", replies[0], replies[1], replies[2],
           replies[3]);
}
  ...
  case VERSION4: {
    ...
    if (methods != 1) {
      ...
      if (is_socks4a(dest_addr.addr)) {
        ...
        inet_fd = socks_connect_domain(
            &(address_t){.addr = domain, .port = dest_addr.port});
      } else {
...
void thread_handling() {
  ...
  while (1) {
    if ((net_fd = accept(server_fd, (struct sockaddr *)&address, &addrlen)) <
        0) {
      log_error("accept");
      exit(EXIT_FAILURE);
    }
...
void print_usage() {
  ...
  printf("  -f, --file FILE         Specify the file for log message (default "
         "is stdout)\n");
  ...
  printf(
      "  -u, --username USER     Specify username (default is \"username\")\n");
  printf(
      "  -w, --password PASS     Specify password (default is \"password\")\n");
...
int main(int argc, char *argv[]) {
  ...
  while ((opt = getopt_long(argc, argv, "hf:p:su:w:SaACc:nN", options,
                            &opt_index)) != -1) {
    switch (opt) {
    case 'h':
      print_usage();
